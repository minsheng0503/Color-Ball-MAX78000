...
const float ars[NUM_ARS] = { 0.95f, 0.85f, 0.40f, 0.25f };
...
{
    int8_t *cl_addr = (int8_t *)0x50c1b000;
...
void localize_objects(void)
{
    ...
    float cxcy[4];
    float xy[4];
    int class_idx, prior_idx, global_prior_idx, cnt;

    nms();
    cnt = 0;
    for (class_idx = 3; class_idx > -1; --class_idx) {
        for (prior_idx = 0; prior_idx < num_nms_priors[class_idx]; ++prior_idx) {
        	if(nms_removed[3][prior_idx] == 1 || nms_removed[2][prior_idx] == 1 || nms_removed[1][prior_idx] == 1){
        		nms_removed[0][prior_idx] = 1;
        	}
        	if (nms_removed[class_idx][prior_idx] != 1) {
                ...
                if((xy[1] > 0.4 && xy[1] < 0.55) && (xy[0] > 0.4 && xy[0] < 0.55)){
                	printf("\n");
                	if(class_idx == 0){
                		printf("%d. RED\n", cnt);
                	}
        	...
        	nms_removed[class_idx][prior_idx] = 1;
